...
from dataBase.db import stats_linea_14dias, stats_por_tipo, stats_barras_anio
...
@app.get("/api/stats/linea14")
def api_stats_linea14():
    datos = stats_linea_14dias()
    return jsonify(datos)

@app.get("/api/stats/torta")
def api_stats_torta():
    datos = stats_por_tipo() 
    return jsonify(datos)

@app.get("/api/stats/barras12m")
def api_stats_barras12m():
    q = request.args.get("anio", type=int)
    datos = stats_barras_anio(q)
    return jsonify(datos)
